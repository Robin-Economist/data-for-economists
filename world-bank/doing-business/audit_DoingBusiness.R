...
meta <- WDIcache()
db_series <- meta$series[meta$series$sourceDatabase == "Doing Business", ]

db_series <- db_series %>%
  mutate(prefix = sub("^([A-Z]+\\.[A-Z]+)\\..*$", "\\1", indicator))

prefix_counts <- db_series %>%
...
categorize_prefix_db <- function(prefix) {
  case_when(
    prefix == "IC.REG" ~ "Starting a Business",
    prefix %in% c("IC.CNST", "IC.DCP") ~ "Construction Permits",
    prefix == "IC.ELC" ~ "Getting Electricity",
    prefix == "IC.CRED" ~ "Getting Credit",
    prefix == "PROT.MINOR" ~ "Protecting Minority Investors",
    prefix %in% c("PAY.TAX", "OTHR.TAX") ~ "Paying Taxes",
    prefix == "TRD.ACRS" ~ "Trading Across Borders",
    prefix == "ENF.CONT" ~ "Enforcing Contracts",
    prefix == "RESLV.ISV" ~ "Resolving Insolvency",
    prefix == "IC.BUS" ~ "General / Composite Business Indicators",
    TRUE ~ "Other / Uncategorized"
  )
}

db_series <- db_series %>%
  mutate(category = categorize_prefix_db(prefix))

category_summary <- db_series %>%
...
# Sauvegarder la classification complète pour le README et pour référence
write.csv(db_series %>% select(indicator, name, prefix, category), 
          "doing_business_categories.csv", row.names = FALSE)