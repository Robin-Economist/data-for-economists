...
meta <- WDIcache()
db_name <- "Statistical Capacity Indicators"
series_codes <- meta$series$indicator[meta$series$sourceDatabase == db_name]

length(series_codes)

all_databases <- unique(meta$series$sourceDatabase)
grep("statistic", all_databases, ignore.case = TRUE, value = TRUE)

# ==============================================================================
# COMPLETENESS AUDIT - Statistical Performance Indicators (SPI), World Bank
# ==============================================================================
#
# Same method as previous audits (WDI, Education Statistics, Doing
# Business), applied to the "Statistical Performance Indicators (SPI)"
# database (~72 series — measures the quality and capacity of countries'
# own statistical systems).
#
# NOTE: this database was formerly called "Statistical Capacity
# Indicators" — it was renamed by the World Bank. Use the exact name
# below when filtering (verified via WDIcache()).
#
...
# ===== 1. Load metadata and target the SPI database =====
meta <- WDIcache()
db_name <- "Statistical Performance Indicators (SPI)"
series_codes <- meta$series$indicator[meta$series$sourceDatabase == db_name]

length(series_codes)  # expect ~72
...
raw_files <- list.files("audit_SPI", full.names = TRUE)
dir.create("audit_detail_SPI", showWarnings = FALSE)

for (f in raw_files) {
  batch_name <- tools::file_path_sans_ext(basename(f))
  out_path <- paste0("audit_detail_SPI/", batch_name, "_detail.rds")
...
cat("Detail files:", length(list.files("audit_detail_SPI")), "\n")
...
detail_files <- list.files("audit_detail_SPI", full.names = TRUE)
dir.create("missing_report_SPI", showWarnings = FALSE)

for (f in detail_files) {
  batch_name <- tools::file_path_sans_ext(basename(f))
  out_path <- paste0("missing_report_SPI/", batch_name, "_report.rds")
...
cat("Report files:", length(list.files("missing_report_SPI")), "\n")


# ===== 6. Export the consolidated report as a single CSV =====
report_files <- list.files("missing_report_SPI", full.names = TRUE)

output_csv <- "missing_values_report_SPI.csv"
...
dir.create("completeness_stats_SPI", showWarnings = FALSE)

# ----- By series -----
for (f in detail_files) {
  batch_name <- tools::file_path_sans_ext(basename(f))
  out_path <- paste0("completeness_stats_SPI/", batch_name, "_by_series.rds")
  if (file.exists(out_path)) next
...
by_series_files <- list.files("completeness_stats_SPI", pattern = "_by_series.rds", full.names = TRUE)
...
write.csv(completeness_by_series, "completeness_stats_by_series_SPI.csv", row.names = FALSE)

# ----- By country -----
for (f in detail_files) {
  batch_name <- tools::file_path_sans_ext(basename(f))
  out_path <- paste0("completeness_stats_SPI/", batch_name, "_by_country.rds")
  if (file.exists(out_path)) next
...
by_country_files <- list.files("completeness_stats_SPI", pattern = "_by_country.rds", full.names = TRUE)
...
write.csv(completeness_by_country, "completeness_stats_by_country_SPI.csv", row.names = FALSE)

# ----- By year -----
for (f in detail_files) {
  batch_name <- tools::file_path_sans_ext(basename(f))
  out_path <- paste0("completeness_stats_SPI/", batch_name, "_by_year.rds")
  if (file.exists(out_path)) next
...
by_year_files <- list.files("completeness_stats_SPI", pattern = "_by_year.rds", full.names = TRUE)
...
write.csv(completeness_by_year, "completeness_stats_by_year_SPI.csv", row.names = FALSE)
...
# Vérifier que les batches SPI existent bien
list.files("audit_SPI")

# Vérifier que le détail SPI a bien été calculé
list.files("audit_detail_SPI")

# Vérifier la variable detail_files actuellement en mémoire
detail_files
...
batch_1 <- readRDS("audit_SPI/batch_1.rds")
colnames(batch_1)
head(batch_1)
...
for (f in raw_files) {
  ...
  long_data <- batch_data %>%
    pivot_longer(
      cols = -c(country, iso3c, year),   # <- iso2c retiré, n'existe pas dans SPI
      names_to = "indicator",
      values_to = "value"
    )
...
for (f in report_files) {
  d <- readRDS(f)
  write.table(d, output_csv, sep = ",", row.names = FALSE,
              col.names = !file.exists(output_csv), append = TRUE)
  rm(d)
  gc()
}
...
for (f in detail_files) {
  ...
  if (file.exists(out_path)) next
  d <- readRDS(f)
  by_series <- d %>% group_by(indicator) %>%
...
for (f in detail_files) {
  ...
  if (file.exists(out_path)) next
  d <- readRDS(f)
  by_country <- d %>% group_by(country, iso3c) %>%
    summarise(total_obs = n(), missing_obs = sum(missing), .groups = "drop")
  saveRDS(by_country, out_path)
  rm(d, by_country); gc()
}
...
cat("\n--- Top 10 LEAST complete series ---\n")
print(as.data.frame(head(completeness_by_series, 10)))
cat("\n--- Top 10 MOST complete series ---\n")
print(as.data.frame(tail(completeness_by_series, 10)))
cat("\n--- Top 10 LEAST complete countries ---\n")
print(as.data.frame(head(completeness_by_country, 10)))
cat("\n--- Top 10 MOST complete countries ---\n")
print(as.data.frame(tail(completeness_by_country, 10)))
...
meta <- WDIcache()
spi_series <- meta$series[meta$series$sourceDatabase == "Statistical Performance Indicators (SPI)", ]

# Voir tous les noms (base petite, on peut tout voir)
spi_series[, c("indicator", "name")] %>% as.data.frame()