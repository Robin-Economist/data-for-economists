# ==============================================================================
# COMPLETENESS AUDIT - Wealth Accounts, World Bank
# ==============================================================================
#
# Same method as previous audits (WDI, Education Statistics, Doing
# Business, SPI, WGI, WWBI), applied to "Wealth Accounts" (146 series —
# measures the components of national wealth: produced capital, natural
...
RAW_DIR    <- file.path(PROJECT_ROOT, "audit_WealthAccounts")
DETAIL_DIR <- file.path(PROJECT_ROOT, "audit_detail_WealthAccounts")
REPORT_DIR <- file.path(PROJECT_ROOT, "missing_report_WealthAccounts")
OUTPUT_DIR <- file.path(WORLD_BANK_FOLDER, "wealth-accounts")
...
cat("Columns in this database:", paste(colnames(sample_batch), collapse = ", "), "\n")
...
output_csv <- file.path(OUTPUT_DIR, "missing_values_report_WealthAccounts.csv")
...
write.csv(completeness_by_series, file.path(OUTPUT_DIR, "completeness_stats_by_series_WealthAccounts.csv"), row.names = FALSE)
...
completeness_by_country <- bind_rows(lapply(detail_files, function(f) {
  ...
  mutate(completeness_rate = round(100 * (1 - missing_obs / total_obs), 1)) %>%
  arrange(completeness_rate)
write.csv(completeness_by_country, file.path(OUTPUT_DIR, "completeness_stats_by_country_WealthAccounts.csv"), row.names = FALSE)
...
completeness_by_year <- bind_rows(lapply(detail_files, function(f) {
  ...
  mutate(completeness_rate = round(100 * (1 - missing_obs / total_obs), 1)) %>%
  arrange(year)
write.csv(completeness_by_year, file.path(OUTPUT_DIR, "completeness_stats_by_year_WealthAccounts.csv"), row.names = FALSE)
